...
def main():
    ...
    spacing_type = GenCustom(
        parse_command='SpacingType.safe_from_str($value, SpacingType.NORMAL)', config_value=GenStr("normal"), python_type="SpacingType")

    casing_type = GenCustom(
        parse_command='Casing.safe_from_str($value, Casing.NORMAL)', config_value=GenStr("normal"), python_type="Casing")

    chunking_type = GenCustom(
        parse_command='ChunkingType.safe_from_str($value, ChunkingType.LAST)', config_value=GenStr("last"), python_type="ChunkingType")
    import_statements = [
    ...
    example_config = str(parent_dir/"example_config.toml")
    config_py = str(parent_dir/"config.py")
    # Generate TOML and write to file
    toml_content = build_toml(builder)
    with open(example_config, "w") as f:
        f.write(toml_content)
    print("Generated example_config.toml successfully")
    python_content = build_python(builder)
